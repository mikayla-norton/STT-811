...
employee_rep <- sqldf("SELECT employees.employeeID AS worker, employee2.employeeID AS boss 
                      FROM employees
                      INNER JOIN employee2
                      WHERE employees.report = employee2.employeeID")

employee_rep <- sqldf("SELECT employees.employeeID AS worker, employee2.employeeID AS boss 
                      FROM employees
                      LEFT JOIN employee2
                      ON employees.report = employee2.employeeID")
